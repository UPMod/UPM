class LightVehiclesMenu: RscControlsGroup
{
	idc = -1;
	text = "";
	x = 0;
	...
	class controls
	{
		class GeneralButton: RscButtonUPM_Tools
		{
			...
			action = QUOTE(if !(GVAR(lightVehiclesGeneral)) then {['OPEN_GENERAL'] call FUNC(debugLightVehiclesHandle);});
		};
		...
		class SpectatorButton: RscButtonUPM_Tools
		{
			...
			periodOver = 0;
			sizeEx = (((((2.42424 / 1.81818) min 1.2) / 1.2) / 25) * 0.4);
			action = QUOTE(if !(GVAR(lightVehiclesSpectator)) then {['OPEN_SPECTATOR'] call FUNC(debugLightVehiclesHandle);});
		};
		...
		class Markers2dButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(lightVehiclesMarkers2d)) then {['OPEN_MARKERS2D'] call FUNC(debugLightVehiclesHandle);});
		};
		class Markers3dButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(lightVehiclesMarkers3d)) then {['OPEN_MARKERS3D'] call FUNC(debugLightVehiclesHandle);});
		};
		class UPMInfoButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(lightVehiclesUPMInfo)) then {['OPEN_UPMINFO'] call FUNC(debugLightVehiclesHandle);});
		};
	};
};