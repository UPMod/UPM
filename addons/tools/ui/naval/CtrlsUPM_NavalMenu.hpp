class NavalMenu: RscControlsGroup
{
	idc = -1;
	text = "";
	x = 0;
	...
	class controls
	{
		class GeneralButton: RscButtonUPM_Tools
		{
			...
			action = QUOTE(if !(GVAR(navalGeneral)) then {['OPEN_GENERAL'] call FUNC(debugNavalHandle);});
		};
		...
		class SpectatorButton: RscButtonUPM_Tools
		{
			...
			periodOver = 0;
			sizeEx = (((((2.42424 / 1.81818) min 1.2) / 1.2) / 25) * 0.4);
			action = QUOTE(if !(GVAR(navalSpectator)) then {['OPEN_SPECTATOR'] call FUNC(debugNavalHandle);});
		};
		...
		class Markers2dButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(navalMarkers2d)) then {['OPEN_MARKERS2D'] call FUNC(debugNavalHandle);});
		};
		class Markers3dButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(navalMarkers3d)) then {['OPEN_MARKERS3D'] call FUNC(debugNavalHandle);});
		};
		class UPMInfoButton: RscButtonUPM_Tools
		{
			...
			periodFocus = 0;
			periodOver = 0;
			action = QUOTE(if !(GVAR(navalUPMInfo)) then {['OPEN_UPMINFO'] call FUNC(debugNavalHandle);});
		};
	};
};