	...
	If (!IsNull _x) then
	{
		If (alive (leader _x)) then
		{
			If (_x getvariable ["UPM_activate_grp_mkr",true]) then
			{
				...
				_markercolor = [side _leader] call FUNC(getColorRgbaSide);
				_drawicon = gettext ([_grp,side _leader] call FUNC(getIconGrptype));
				...
				If (_grp getvariable ["UPM_activate_wp_mkr",true]) then
				{
					...
					If (!(_waypoints isequalto [])) then
					{
						...
						If (!(_wppos isequalto [0,0,0])) then
						{
							_behaviourcolor =  [side _leader] call FUNC(getColorRgbaBehaviour);
